...
const getRawTorInfo = (controller: any, keyword: string) => new Promise<string>((resolve, reject) => {
  if (!controller || !controller.socket || !Array.isArray(controller._stack)) {
    reject(new Error('Tor controller is not ready'))
    return
  }

  controller._stack.unshift({
    method: 'RAWGETINFO',
    callback: (error: Error | null, lines: string[]) => {
      if (error) {
        reject(error)
        return
      }
      ...
      const rawLine = lines.find(line => line.startsWith(prefix)) || lines[0] || ''
      resolve(rawLine.startsWith(prefix) ? rawLine.slice(prefix.length) : rawLine)
    }
  })
  controller.socket.write(`GETINFO ${keyword}\r\n`)
})
...
const waitForTorBootstrap = async (controller: any, maxAttempts = 12, delayMs = 5000) => {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const bootstrapStatus = await getRawTorInfo(controller, 'status/bootstrap-phase')
...
const notifyTorStartupPhase = (phase: string) => {
  if (!win || win.isDestroyed()) {
    return
  }

  win.webContents.send(eventConstants.torStartupPhase, { phase })
}
  ...
  if (!torStartupPromise) {
    ...
    torStartupPromise = startTor()
      ...
      .catch(async (error: Error) => {
        ...
        stopTorController()
        notifyTorStartupPhase('bridging')
